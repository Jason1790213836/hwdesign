...
\begin{enumerate}
\item \qtag{Propagation delay} 
\emph{Propagation delay}:   A signal $x$ has time-value change points
\[
    (10, 0), (20, 4), (30, 2)
\]
...
\item \qtag{Critical path} \emph{Critical path}: Consider the following SystemVerilog code:
...
\item \qtag{Splitting paths} \emph{Splitting paths over multiple cycles}: Consider the following
SystemVerilog code:
\begin{systemverilogcode}
  always_comb begin
      act_in = w1*xreg+b1;
