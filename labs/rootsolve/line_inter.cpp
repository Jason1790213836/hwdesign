...
#pragma HLS INTERFACE ap_ctrl_none port=return
    ...
    // Transfer command parameters to local arrays/scalars
    float a[ndim], b[ndim], uab[ndim];
...
#pragma HLS ARRAY_PARTITION variable=x complete dim=2
    ...
    // Run the main intersection computation
compute_loop:
...
#pragma HLS UNROLL
    ...
    // Write output samples back to output stream
    float32_array_utils::write_axi4_stream<WORD_BW>(out_stream, dsq, false, nsamp);
