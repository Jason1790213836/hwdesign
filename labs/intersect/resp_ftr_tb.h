#ifndef RESP_FTR_TB_H
#define RESP_FTR_TB_H
...
#include "include/intersect_error_tb.h"

#define PYSILICON_ENABLE_RESP_FTR_TB_H_MEMBERS
#include "resp_ftr.h"
#undef PYSILICON_ENABLE_RESP_FTR_TB_H_MEMBERS
...
inline void RespFtr::dump_json(std::ostream& os, int indent, int level) const {
    ...
    os << "\"nelem_read\": ";
    os << static_cast<unsigned long long>(this->nelem_read);
    os << ",";
    os << "\n";
    for (int i = 0; i < (level + 1) * step; ++i) { os << ' '; }
    os << "\"error\": ";
...
inline void RespFtr::load_json(const std::string& json_text, size_t& pos) {
    ...
    bool seen_root_nelem_read = false;
    ...
    streamutils::json_expect_char(json_text, pos, ':');
    if (key == "nelem_read") {
        seen_root_nelem_read = true;
        this->nelem_read = static_cast<ap_uint<16>>(static_cast<unsigned long long>(streamutils::json_parse_number(json_text, pos)));
    }
    else if (key == "error") {
    ...
    if (!seen_root_nelem_read) {
    throw std::runtime_error("Malformed JSON: missing required key 'nelem_read'.");
    }
    if (!seen_root_error) {
